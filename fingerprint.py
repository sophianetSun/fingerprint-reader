...
def add_fingerprint_first(user_id='', user_privilege=1):
    id_high, id_low = idToByte(user_id)
...
def upload_dsp_by_id(user_id, eigenvalue):
    id_high, id_low = idToByte(user_id)
...
assert bytes([0xF5, 0x2D, 0, 0x01,
              0, 0, 0x2D^0, 0xF5]) == fingerprint_mode('set', False), \
    'Set Fingerprint add mode prohibit repeat'
assert bytes([0xF5, 0x2D, 0, 0,
              0x01, 0, 0x2D^0, 0xF5]) == fingerprint_mode('read'), \
    'Read Fingerprint add mode'
assert bytes([0xF5, 0x01, 0, 0,
              0x01, 0, 0x01^0, 0xF5]) == add_fingerprint_first('0'), \
    'Add first Fingerprint should be correct'
assert bytes([0xF5, 0x02, 0, 0,
              0x01, 0, 0x02^0, 0xF5]) == add_fingerprint_second('0'), \
    'Add second Fingerprint should be correct'
assert bytes([0xF5, 0x03, 0, 0,
              0x01, 0, 0x03^0, 0xF5]) == add_fingerprint_third('0'), \
    'Add third Fingerprint should be correct'
assert bytes([0xF5, 0x04, 0, 0,
              0, 0, 0x04^0, 0xF5]) == del_specified_user('0'), \
    'Delete specified user by user id'
assert bytes([0xF5, 0x05, 0, 0,
              0, 0, 0x05^0, 0xF5]) == del_all_users(), \
    'Del all users'
assert bytes([0xF5, 0x0B, 0, 0,
              0, 0, 0x0B^0, 0xF5]) == compare_by_id('0'), 'compare with 0 should be correct'
